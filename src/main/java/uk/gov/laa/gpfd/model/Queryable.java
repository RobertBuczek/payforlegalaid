 ...
 */
public interface Queryable<Q, T extends Queryable<Q, T>> {
     ...
     */
    default Collection<Mapping> extractAllMappings() {
        @SuppressWarnings("unchecked")
        T self = (T) this;
        return Queryable.processor(self)
                .allMappings();
     ...
     */
    static <T extends Queryable<Q, T>, Q> QueryProcessor<T, Q> processor(T source) {
     ...
     * @param <Q> the type of query objects
     */
    abstract class QueryProcessor<T extends Queryable<Q, T>, Q>  {
        ...
        QueryProcessor(T source) {
            ...
            this.mapper = q -> {
                if (q instanceof ReportQuery) {
                    return ((ReportQuery) q).getQuery();
                }
                return null;
            };
        }
